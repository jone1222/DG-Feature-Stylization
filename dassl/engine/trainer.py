...
class FeatureStylizationNet(nn.Module):
    ...
    def __init__(self, cfg, model_cfg, num_classes, **kwargs):
        ...
        self.classifier = None
        if num_classes > 0:
            self.classifier = nn.Linear(fdim, num_classes, bias=model_cfg.CLASSIFIER.BIAS)
    ...
    def get_feature(self, x, domain):
        f = self.backbone(x, domain)
        if self.head is not None:
            f = self.head(f)
    ...
    def forward(self, x, return_feature=False, stylization_layer_idx = 2):
        ...
        y = self.classifier(f)
        y_tr = self.classifier(f_tr)

        if return_feature:
            return (y, f), (y_tr, f_tr)
